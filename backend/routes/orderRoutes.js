...
async function insertAttributes(order_item_id, attributes) {
  ...
  for (const attr of attributes) {
    ...
    // Resolve attribute_id from name
...
router.post("/create", async (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Order placed!", order_id, order_item_id });
...
/* ========================================================
   FILE UPLOADS
======================================================== */
router.post("/upload/single/:id", upload.single("file1"), async (req, res) => {
  if (!req.file) return res.status(400).json({ success: false, message: "No file uploaded" });

  const filePath = `/uploads/orderfiles/${req.file.filename}`;

  await db.promise().query("UPDATE orderitems SET file1=? WHERE order_item_id=?", [
    filePath,
    req.params.id
  ]);
...
router.post("/upload/double/:id", upload.fields([{ name: "file1" }, { name: "file2" }]), async (req, res) => {
  const file1 = req.files["file1"]?.[0] ? `/uploads/orderfiles/${req.files["file1"][0].filename}` : null;
  const file2 = req.files["file2"]?.[0] ? `/uploads/orderfiles/${req.files["file2"][0].filename}` : null;

  await db.promise().query(
    "UPDATE orderitems SET file1=?, file2=? WHERE order_item_id=?",
    [file1, file2, req.params.id]
  );

  res.json({ success: true, file1, file2 });
});
  ...
  try {
    ...
    const merged = items.map(item => ({
      ...
      details: attrs
        ...
        .reduce((obj, row) => ({ ...obj, [row.attribute_name]: row.attribute_value }), {})
    }));
...
/* ========================================================
   GET ORDER BY ID (single final version)
======================================================== */
  ...
  try {
    ...
    const [items] = await db.promise().query(
      `SELECT 
        ...
        oi.estimated_price, oi.file1, oi.file2
       FROM orderitems oi
       JOIN products p ON oi.product_id=p.product_id
       WHERE oi.order_id=?`,
      [req.params.id]
    ...
    for (let item of items) {
      ...
      item.details = attrs.reduce((acc, a) => ({ ...acc, [a.attribute_name]: a.attribute_value }), {});
      item.files = [item.file1, item.file2].filter(Boolean);
    }
...
router.put("/:id/status", async (req, res) => {
  ...
  try {
    ...
    if (status !== "Completed") {
      return res.json({ success: true, message: "Status updated (no sales entry)." });
    }

    const [exists] = await db.promise().query(
      "SELECT id FROM sales WHERE order_item_id=? LIMIT 1",
      [id]
    );
